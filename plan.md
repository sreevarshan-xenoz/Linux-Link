...
7. 🟡 **Hyprland window/workspace picker + single-window streaming** — steps 1–3 done 2026-09-20: (1) `server/src/hyprland.rs` raw IPC client (j/clients, j/activewindow, j/monitors, socket2 events; verified live on 0.56.2); (2) window list over the wire — `WindowsPlugin` answers `kdeconnect.linuxlink.windows` with visible windows incl. **monitor-local `local_at`** (crop-rect origin in capture coords), `monitor_size`, and the layout `screen` box (normalized-input remap space); (3) capture crop — `InputPacket::WindowCrop` (tag 8, monitor-local rect; all-zero clears) intercepted by the stream monitor task, applied CPU-side per-frame in the encode task (`VideoFrame::crop_region`), and the encoder is **rebuilt at the resulting resolution** so the negotiated window size always reaches the pipeline (also self-heals preset-vs-capture mismatch). Caveat: socket1 *write* dispatchers broken upstream since 0.56's Lua IPC rewrite (hyprwm/Hyprland#16224) — actions ride input injection. Remaining: Android picker UI + `sendWindowCrop` bridge + letterbox-accurate absolute mapping. A moat feature: Sunshine has no per-window Linux capture
