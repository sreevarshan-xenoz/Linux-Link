...
7. 🟡 **Hyprland window/workspace picker + single-window streaming** — step 1 done 2026-09-20: `server/src/hyprland.rs` raw IPC client (j/clients, j/activewindow, socket2 events; verified live on 0.56.2). Caveat found: socket1 *write* dispatchers (`dispatch`/`keyword`) broken upstream since the 0.56 Lua IPC rewrite (hyprwm/Hyprland#16224) — actions must go through input injection instead. Remaining: window list over the wire, capture crop to window geometry, Android picker UI — a moat feature: Sunshine has no per-window Linux capture
