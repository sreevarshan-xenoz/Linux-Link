...
- 2026-09-20: Tier 1 partial — Stats HUD (`stream/StatsHud.kt`, polls `RustCore.streamingStats()`) and desktop Shortcut bar (`stream/ShortcutBar.kt`: Super, Alt+Tab, Ctrl+Alt+Del, PrtSc, Esc, Super+1..9 Hyprland workspaces) landed, plus a full Android→evdev keycode table fix in the bridge (commit ba26e6c — old passthrough sent wrong keys for modifiers/digits). Tier 1 #2 (touch input modes) is BLOCKED on R2#4: the QUIC wire only carries relative `MouseMove{dx,dy}`, so direct-touch absolute needs an absolute packet + `/dev/uinput` server-side. Input helpers live on `RustCore` (`tapKey`, `holdKey`, `releaseKey`, `hotkey`, `sendText`).
- Next: foreground-service + notification + auto-connect (Tier 1 #5) and clipboard UX (#4), then R2#4 (`/dev/uinput` absolute input + absolute-coordinate packet, which unblocks direct-touch), then remaining R2 items (#3 in-process `ffmpeg-next`, #5 quinn CC tuning, #6 VFR pacing); R1 iroh spike after the R2 batch.
