...
- Next: wire `RemoteDesktopView` + `RustCore` into Compose screens (connection, remote desktop, file browser, settings); then continue roadmap R2 items (#3 in-process `ffmpeg-next` encode, #4 `/dev/uinput` absolute input, #5 quinn CC tuning, #6 VFR pacing).
