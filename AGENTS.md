...
- 2026-09-20: R4 (plan: `docs/rustdesk-research-roadmap.md`) — landed: A1 link-path reporting, A2 session outcome telemetry (`linux-link sessions`), A3/E5 relay bitrate floor + full-quality override, B1 wlroots `zwlr_screencopy` capture backend (the only item verified live, on this Hyprland box), D1 server-enforced view-only mode.
- Next: R4 B2 (window-granular screencopy, riding the B1 backend). Parked: R2#3 remainder (in-process VAAPI/NVENC — untestable on this box).
